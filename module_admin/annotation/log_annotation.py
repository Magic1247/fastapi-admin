...
def log_decorator(title: str, business_type: int, log_type: Optional[str] = 'operation'):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            # 获取请求的ip及ip归属区域
            oper_ip = request.headers.get("X-Forwarded-For") if request.headers.get("X-Forwarded-For") else '127.0.0.1'
            oper_location = '内网IP'
            if AppConfig.app_ip_location_query:
                try:
                    if oper_ip != '127.0.0.1' and oper_ip != 'localhost':
                        ip_result = requests.get(
                            f'https://ip.taobao.com/outGetIpInfo?ip={oper_ip}&accessKey=alibaba-inc').json()
                        logger.info(ip_result)
                        if ip_result['code'] == 0:
                            region = ip_result['data']['region']
                            city = ip_result['data']['city']
                            isp = ip_result['data']['isp']
                            if region is not None and city is not None:
                                oper_location = f'{region}-{city}-{isp}'
                            else:
                                oper_location = '未知'
                        else:
                            oper_location = '未知'
